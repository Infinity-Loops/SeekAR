...
public class PumpItPowerup : MonoBehaviour, IPointerDownHandler
{
        ...
        if (MapUtils.isCloseTo(transform.position, radius) || PowerupData.whaleActive)
        {
            if (!alreadyTap)
            {
                ...
                transform.DOMove(targetLoc, 0.25f).OnComplete(() =>
                {
                    ...
                    {
                        transform.DOScale(Vector3.zero, 0.25f).SetDelay(4).OnComplete(() =>
                        {
                            PowerupData.pumpItActive = true;
                            PowerupNotificationHandler.instance.HandleNotificationSequence("Pump it!", "The market is pumping! all values of collected tokens are increased for 5 minutes.", 5f);
                            Destroy(gameObject);
                        });
                    });
                });
