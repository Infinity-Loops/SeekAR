    ...
    public void ReplacePhoto()
    {
        ...
        RequestPermissionAsynchronously(() =>
        {
            NativeGallery.Permission permission = NativeGallery.GetImageFromGallery((path) =>
            {
                ...
                {
                    Texture2D imageParsed = NativeGallery.LoadImageAtPath(path, -1, false, true, false);
                    Texture2D cutTex = CutTextureToSquare(imageParsed, imageParsed.width);
                    DataSystem.avatar = Sprite.Create(cutTex, new Rect(Vector2.zero, new Vector2(cutTex.width, cutTex.height)), new Vector2(0.5f, 0.5f));
                    avatarIcon.sprite = DataSystem.avatar;
